...
namespace AHD.SM.Formats
{
    public class MPlayer : SubtitlesFormat
    {
        ...
        public override void Load(string filePath, Encoding encoding)
        {
            ...
            for (int i = 0; i < Lines.Length; i++)
            {
                try
                {
                    ...
                    Sub.Text = SubtitleText.FromString(TextLines[3].Replace("|", "\n"));
        ...
        public override void Save(string filePath, Encoding encoding)
        {
            ...
            {
                string CurrentLine = TimeFormatConvertor.To_Frame(this.SubtitleTrack.Subtitles[i].StartTime, FrameRate) + "," +
                    TimeFormatConvertor.To_Frame(this.SubtitleTrack.Subtitles[i].EndTime, FrameRate) + ",0," +
